...
# %%
def calculate_weights_2(Y_cal, Y_test, t_test):

    Y_cal_times = Y_cal.coords[2, :].astype(np.int64)  # Time indices of calibration points
    weights = np.zeros(len(Y_cal.data))  # Placeholder for weights

    for idx, t_cal in enumerate(Y_cal_times):
        # Temporal proximity component
        temporal_proximity = np.abs(t_cal - t_test)

        # Find mutual time points between calibration and test trajectories
        mutual_times_cal = np.where(Y_cal.coords[2, :] == t_cal)[0]  # Calibration indices
        mutual_times_test = np.where(Y_test.coords[2, :] == t_cal)[0]  # Test indices

        if len(mutual_times_cal) > 0 and len(mutual_times_test) > 0:
            # Ensure alignment of mutual times for both calibration and test
            observed_values_cal = Y_cal.data[mutual_times_cal]
            observed_values_test = Y_test.data[mutual_times_test]
            
            # Take only the minimum length for comparison to avoid broadcasting issues
            min_length = min(len(observed_values_cal), len(observed_values_test))
            trajectory_similarity = np.mean(
                np.abs(observed_values_cal[:min_length] - observed_values_test[:min_length])
            )
        else:
            # Assign a large value if no mutual times
            trajectory_similarity = float('inf')

        # Calculate the weight as the product of temporal proximity and trajectory similarity
        weights[idx] = temporal_proximity * trajectory_similarity

    # Normalize the weights
    weights_normalized = weights / np.sum(weights)
    return weights_normalized
    
t_test = 500  # Example test time point
weights_normalized = calculate_weights_2(Y_cal, Y_test, t_test)
print("Normalized Weights:", weights_normalized)
...
# %%
def adjust_alpha_tilde_weighted_sACI_2(Y_cal, Y_cal_hat, nonconformity_scores, t_test, alpha, delta, alpha_tilde_int):
...
# %%
# # Example usage
# t_test = 600  # Test time point
# alpha = 0.1
# delta = 0.01
# alpha_tilde_int = 0.2

# # Adjust alpha_tilde for the given t_test
# alpha_tilde = adjust_alpha_tilde_weighted_sACI_2(
...
def prediction_intervals_weighted_ACI_all(Y_cal, Y_cal_hat, Y_test_hat, nonconformity_scores, alpha, delta):
    ...
    # Extract coordinates and data from Y_test_hat
    Y_test_coords = Y_test_hat.coords
    Y_test_data = Y_test_hat.data

    # Extract time coordinates from Y_cal
    Y_cal_times = Y_cal.coords[2, :].astype(np.int64)

    # Loop through unique test time points
    unique_t_test = np.unique(Y_test_coords[2, :])  # Unique time points in Y_test_hat

    for t_test in unique_t_test:
        # Compute weights for all calibration points for the current test time
        weights_normalized = calculate_weights_2(Y_cal, Y_test_hat, t_test)
...
for iteration in range(num_iterations):
    ...
    data = generate_data(I, J-1, T, idx_x, idx_y_train, R, k, N)
    result = dlts(data[0], data[1], I, J, T, idx_x, idx_y_train, R, k, N)

    weights = result[0]
    F = result[1]

    # Predictions
    Y_cal_hat = y_pred(weights, data[2], F, I, idx_y_cal, R, k)
    Y_test_hat = y_pred(weights, data[2], F, I, idx_y_test, R, k)
...
# %%
# Initialize arrays to store coverage rates and average lengths
coverage_rates = []
average_lengths = []
...
for iteration in range(num_iterations):
    ...
    # Predict calibration and test data
    Y_cal_hat = y_pred(weights, knots, F, I, idx_y_cal, R, k)
    Y_test_hat = y_pred(weights, knots, F, I, idx_y_test, R, k)
    ...
    # Calculate coverage rate and average interval length
    coverage_rate = calculate_coverage_rate(Y_test, prediction_intervals_dict)
    average_length = calculate_average_interval_length(prediction_intervals_dict)
    ...
    print(f"Iteration {iteration + 1}: Coverage Rate = {coverage_rate:.4f}, Average Length = {average_length:.4f}")

# Calculate and print the mean and standard deviation of coverage rates and average lengths
coverage_mean = np.mean(coverage_rates)
coverage_std = np.std(coverage_rates)
length_mean = np.mean(average_lengths)
length_std = np.std(average_lengths)

print("\nSummary of 50 Experiments:")
print(f"Coverage Rate - Mean: {coverage_mean:.4f}, Std Dev: {coverage_std:.4f}")
print(f"Average Interval Length - Mean: {length_mean:.4f}, Std Dev: {length_std:.4f}")
